...
<xsl:stylesheet
  ...
  <xsl:template name="portal.page.bar.link.logout">
    <xsl:if test="$AUTHENTICATED='true'">
     <li class="link-logout">
        <a href="/Shibboleth.sso/Logout" title="{upMsg:getMessage('log.off.and.exit', $USER_LANG)}">
  ...
  <xsl:template name="external.login">
    <div id="portalCASLogin" class="fl-widget-content">
        <a id="portalCASLoginLink" class="button" href="/uPortal/Login" title="Shibboleth Login">
            <span>USC Login</span>
        </a>
    </div>
  </xsl:template>
